...
func findSeatFromSortedSeats(sortedSeats: seq[int]): int =
    var seatNum: int = 0
    for i in 0..sortedSeats.len()-2:
        if (sortedSeats[i+1] - sortedSeats[i]) != 1:
            seatNum = sortedSeats[i]+1
    seatNum

proc calculateAnwerFromSeatIds(allSeatIds: seq[int]): int =
    var sortedSeats = allSeatIds
    sortedSeats.sort()
    findSeatFromSortedSeats(sortedSeats)

proc BinaryBoarding*(fileName: string): int =
    let
        inputBoardingSeq: seq[string] = getInputFromFile(fileName)
    var
        allSeatIds: seq[int] = calculateAllBoardingInput(inputBoardingSeq)
