...
func validate_byr(fieldVal: string): bool =
    fieldVal.parseInt >= 1920 and fieldVal.parseInt <= 2002

func validate_iyr(fieldVal: string): bool =
    fieldVal.parseInt >= 2010 and fieldVal.parseInt <= 2020

func validate_eyr(fieldVal: string): bool =
...
func validate_hgt(fieldVal: string): bool =
    if "cm" in fieldVal:
...
func validate_hcl(fieldVal: string): bool =
    match(fieldVal, re2"\#[[:xdigit:]]{6}")

func validate_ecl(fieldVal: string): bool =
    let validEyeColors = @["amb", "blu", "brn", "gry", "grn", "hzl", "oth"]
    fieldVal in validEyeColors

func validate_pid(fieldVal: string): bool =
    match(fieldVal, re2"[[:xdigit:]]{9}")

func hasAllRequiredKeys(passportTable: Table[string, string]): bool =
...
func validatePassportAgainstFields(passportTable: Table[string, string]): bool =
    var isValid = true
...
func validatePassportTableWithValidKeys(passportTable: seq[Table[string,
        string]]): int =
    var answer: int
